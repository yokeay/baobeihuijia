              ...
              <tbody>
                {items.map((q: any) => (
                  <tr key={q.id} className={trClass}>
                    ...
                    <td className={td}>
                      {q.caseName ? (
                        <button onClick={() => { setDrawerCase(q.caseData); setDrawerOpen(true); }}
                          className="text-blue-600 dark:text-blue-400 hover:underline text-left">
                          {q.caseName}
                        </button>
                      ) : "-"}
                    </td>
                    <td className={td} style={{ maxWidth: 250 }}><div className="truncate">{q.content}</div></td>
                    <td className={td}>
                ...
                {items.length === 0 && (
                  <tr><td colSpan={6} className="text-center py-8 text-gray-400 dark:text-gray-600">暂无疑问</td></tr>
                )}
              </tbody>
            </table>
          </div>
