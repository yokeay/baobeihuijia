...
import { RefreshIcon, CheckIcon, CloseIcon, LightbulbIcon } from "@/components/ui/Icon";
...
export default function CluesPage() {
  ...
  return (
    <div>
      <PageHeader
        title="线索管理"
        description="用户提交的寻人线索，审核通过后展示在对应寻亲帖下"
        actions={
      ...
      <Panel>
        {loading ? (
          ...
          <EmptyState title="暂无线索" icon={<LightbulbIcon size={20} />} />
        ) : (
          <>
            <Table>
              <thead>
              ...
              <tbody>
                {items.map((c: any) => (
                  <Tr key={c.id}>
                    <Td className="font-medium whitespace-nowrap">{c.submitterName || "匿名"}</Td>
                    <Td>
                      {c.caseName ? (
                        <button
                          type="button"
                          onClick={() => { setDrawerCase(c.caseData); setDrawerOpen(true); }}
                    ...
                    <Td>
                      <Badge tone={statusTone(c.status)}>{STATUS_LABELS[c.status] || c.status}</Badge>
                    </Td>
