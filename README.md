...
天色按访客的**本地时间**走：06:00–18:00 是白天的近白浅蓝天，只留十来颗压淡的星，白色流星照样划；其余时间是夜空 —— 星云辉光、130 颗闪烁的星点、周期性划过的流星，整页铺满到最下沿，不留过渡色带。刷新后**第一次向下滚动**会以 PPT 式的淡出 / 淡入整屏翻到寻人列表；翻完这一次就交还给原生滚动，上下滚动都是连续跟手的。首页整页不显示滚动条（照常可滚）。
...
>
> 脚本强制 `reduced_motion="reduce"`，所以首页会截成天空的静态一帧 —— 首次滚动的整屏翻页在减少动效下按设计不启用，截图脚本验证不了它。截图时天色跟着**跑脚本那台机器的本地时间**走，夜里截出来是夜空、白天是浅蓝天。`SHOT_BASE` 可指定截图目标地址，默认 `http://localhost:3000`。
...
src/
├── app/
│   ├── page.tsx              首页（昼夜天空首屏 + 侧栏筛选 + 瀑布流，含首次滚动的整屏翻页）
